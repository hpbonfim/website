...
&& ansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name}' ansible/install_jenkins.yaml
EOF
  }
  tags = {
...
&& ansible-playbook --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name} master_ip=${aws_instance.jenkins-master.private_ip}' ansible/install_worker.yaml
EOF
  }
  tags = {
